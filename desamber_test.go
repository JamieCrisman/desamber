	...
	for _, tc := range tt {
		t.Run(tc.Name, func(t *testing.T) {
			...
			if result.Century != tc.Expected.Century {
				t.Errorf("Expected century to be %s, but got %s", tc.Expected.Century, result.Century)
			}
			if result.Year != tc.Expected.Year {
				t.Errorf("Expected year to be %s, but got %s", tc.Expected.Year, result.Year)
			}
			...
			if result.Day != tc.Expected.Day {
				t.Errorf("Expected day to be %s, but got %s", tc.Expected.Day, result.Day)
			}
		})
	}
}
